...
$(document).ready(function(){
  ...
  $('#btnprint').on('click',(function(e){
    ...
    var inijson = {
        'value1' : $('[name="val_data1"]').val(),
        'value2' : $('[name="val_kandang1"]').val(),
        'value3' : $('[name="daydari"]').val(),
        'value4' : $('[name="daysampai"]').val(),
        'value5' : $('[name="val_periode1"]').val(),
        'value6' : $('[name="val_kandang2"]').val(),
        'value7' : $('[name="val_periode2"]').val(),
        'value8' : $('[name="val_data2"]').val(),
        };

    $.ajax({
        url : "<?php echo base_url('export_excel/ydhistory_house_day');?>",
        type: "POST",
        data: inijson,
        dataType:"JSON",
...
function loadtabel() {
  ...
  $("#mytable").DataTable({
    ...
    serverSide:true,
    ajax:{
        url:"<?php echo base_url('report/datatabel_dyaxisd');?>",
        type:"POST",
        data : {
...
function cekgrafik() {
  ...
  if (parseInt($('[name="daydari"]').val()) > parseInt($('[name="daysampai"]').val())) {title = 'Data grow day salah! Mohon set ulang';cek = 1;}
...
function grafik(){
  ...
  var inijson = {
    ...
    $.ajax({
      type: "POST",
      url : "<?php echo base_url('report/dataaxisd'); ?>",
      data : inijson,
      dataType : "JSON",
      success : function(data){
        get_sess(data.sess);
        if(data.status == true){
          ...
          $('#mytable').DataTable().ajax.url("<?php echo base_url('report/datatabel_dyaxisd');?>").load();
        }else{
