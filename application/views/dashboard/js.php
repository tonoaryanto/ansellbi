...
$(document).ready(function(){
  $('#btnprint').on('click',(function(e){
    ...
    $.ajax({
        url : "<?php echo base_url('export_excel/histori_house_hour');?>",
        type: "POST",
        data: {
...
function loadtabel() {
  ...
  $("#mytable").DataTable({
    ...
    serverSide:true,
    ajax:{
        url:"<?php echo base_url('histori_house/datatabel');?>",
        type:"POST",
        data : {
        'value1' : kategrafik,
...
if ($('#optionselect').val() != '' && $('#optionselect_kandang').val() != ''){
  grafik();
}
});
...
function grafik(){
  ...
  $.ajax({
    type: "POST",
    url : "<?php echo base_url('histori_house/datajson/'); ?>",
    data : {
      'value1' : kategrafik,
      'value2' : datgraf,
      'value3' : datkandang,
    ...
    success : function(data){
      ...
      if(data.status == true){
        ...
        $('#mytable').DataTable().ajax.url("<?php echo base_url('histori_house/datatabel');?>").load();
      }else{
...
function selectdata_kandang(){
  var inidata = $.ajax({
    type: "POST",
    url : "<?php echo base_url('histori_house/data_select_kandang'); ?>",
    data : {
      'value1' : kategrafik,
    },
    dataType: "JSON",
...
function selectdata(){
  var inidata = $.ajax({
    type: "POST",
    url : "<?php echo base_url('histori_house/data_select'); ?>",
    data : {
      'value1' : kategrafik,
